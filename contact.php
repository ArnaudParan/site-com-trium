<div class="row">
	<div class="col-lg-offset-3 col-lg-6 bloc_contact">
		<div class="row">
			<div class="col-lg-4" style="background-color:black">
				<img src="/logos/logo_trium.png" alt="Étudiants"/>
			</div>
			<div class="col-lg-8">
				<h1>Vous êtes étudiant?<br/>
				Posez-nous vos questions sur twitter</h1>
			</div>
		</div>
		<div class="lisere_bleu"></div>
	</div>
</div>
<div class="row">
	<div class="col-lg-offset-3 col-lg-6 bloc_contact">
		<div class="row">
			<div class="col-lg-4 col-lg-push-8" style="background-color:black">
				<img src="/logos/logo_trium.png" alt="Entreprises"/>
			</div>
			<div class="col-lg-8 col-lg-pull-4">
				<h1>Pour les entreprises souhaitant participer au forum:</h1>
				<a href="mailto:contact@forum-trium.com">contact@forum-trium.com</a>
			</div>
		</div>
		<div class="row" style="postition:absolute;bottom:0;"><div class="lisere_bleu"></div></div>
	</div>
</div>
<div class="row">
	<div class="col-lg-offset-3 col-lg-6 bloc_contact">
		<div class="row">
			<div class="col-lg-4" style="background-color:black">
				<img src="/logos/logo_trium.png" alt="Organisateurs"/>
			</div>
			<div class="col-lg-8">
				<h1>Organisateurs:</h1>
				<address>
					<strong>Forum Trium</strong><br/>
					<div class="row" style="postition:absolute;bottom:0;">6-8 Avenue Blaise Pascal<br/></div>
					77455 Champs sur Marne
				</address>
			</div>
		</div>
		<div class="lisere_bleu"></div>
	</div>
</div>
<div class="row">
	<div class="col-lg-offset-3 col-lg-6 bloc_contact">
		<div class="row">
			<div class="col-lg-4 col-lg-push-8" style="background-color:black">
				<img src="/logos/logo_trium.png" alt="Organisateurs"/>
			</div>
			<div class="col-lg-8 col-lg-pull-4">
				<h1>Organisateurs:</h1>
				<address>
					<strong>Forum Trium</strong><br/>
					6-8 Avenue Blaise Pascal<br/>
					77455 Champs sur Marne
				</address>
			</div>
		</div>
		<div class="lisere_bleu"></div>
	</div>
</div>
